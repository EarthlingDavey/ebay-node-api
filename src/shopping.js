...
const getAllCategories = function (categoryID) {
  const requestURL = `${urlObject.buildShoppingUrl(
    this.options,
    'GetCategoryInfo'
  )}&${stringifyUrl({ CategoryID: categoryID || -1 })}`;
...
const getUserDetails = function (input) {
  ...
  if (!input.userId)
    throw new Error('invalid_request_error -> userId is null or invalid');
  input.includeSelector = input.includeSelector
    ? input.includeSelector
    : 'Details';
  const requestUrl = `${urlObject.buildShoppingUrl(
    this.options,
    'GetUserProfile'
...
const getItemStatus = function (itemIds) {
  if (!itemIds)
    throw new Error('invalid_request_error -> Item ID is null or invalid');
  const paramsObj = {
...
const getShippingCosts = function (input) {
  ...
  if (!input.itemId)
    throw new Error('invalid_request_error -> Item id is null or invalid');
  const url = `${urlObject.buildShoppingUrl(
    this.options,
    'GetShippingCosts'
  )}&${stringifyUrl(input)} `;
...
const getMultipleItems = function (options) {
  ...
  const requestUrl = `${urlObject.buildShoppingUrl(
    ...
    itemId: makeString(options.itemId, { braces: 'false', quotes: 'no' }),
  })}`;
  return getRequest(requestUrl).then(
    (data) => {
      return JSON.parse(data);
